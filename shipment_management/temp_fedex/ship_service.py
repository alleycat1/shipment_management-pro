...
# TODO - (We should remove ship-service.py from our source code and import from fedex in python libs)
# TODO - After Pull Request will be merged - https://github.com/python-fedex-devs/python-fedex/pull/84
...
# ###################################################################
# Fedex International Shipment Requrest Class #######################
# ###################################################################
class FedexProcessInternationalShipmentRequest(FedexBaseService):
    """
    This class allows you to process (create) a new International FedEx shipment. You will
    need to populate the data structures in self.RequestedShipment, then
    send the request. Label printing is supported and very configurable,
    returning an ASCII representation with the response as well.
    Note: The current limitation is limited to sending Commodity goods only. No document sending supported
    """
    ...
    def __init__(self, config_obj, *args, **kwargs):
        ...
        """@ivar: Holds the RequestedShipment WSDL object."""
        # Call the parent FedexBaseService class for basic setup work.
        super(FedexProcessInternationalShipmentRequest, self).__init__(
            self._config_obj, 'ShipService_v17.wsdl', *args, **kwargs)
    ...
    def _prepare_wsdl_objects(self):
        ...
        self.RequestedShipment = self.client.factory.create('RequestedShipment')
        self.RequestedShipment.ShipTimestamp = datetime.datetime.now()
        # INTERNATIONAL PRIORITY SHIPMENT SERVICE ASSUMED BY DEFAULT
        self.RequestedShipment.ServiceType = "INTERNATIONAL_PRIORITY"
